...
* **Audio reliably starts on join, even after a slow first cold-start** (PR [#526](https://github.com/Psychotoxical/psysonic/pull/526)). Previously, when the initial sync's 5 s ready-poll timed out (slow Navidrome warmup), the next pull tick took the cheap "track already loaded" shortcut and fired `seek` + `resume` on a stuck engine — `seek` no-oped and `resume` couldn't kick a track that never started. Guest stayed silent until the next host track-change. The shortcut is now gated on engine state matching the host's expected state; the limbo case falls through to a fresh `playTrack` that re-initialises the engine cleanly.
