...
* On Linux/PipeWire, the active default is resolved from the real sink (not cpal's generic `"Default Audio Device"` alias), so device changes are detected and EQ profiles bind to the correct output.
