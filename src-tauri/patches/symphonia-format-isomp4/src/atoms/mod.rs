...
impl<R: ReadAtom> AtomIterator<R> {
    ...
    where
        R: MediaSource,
    {
        // Must currently have a pending atom to allow resynchronizing to it, unless the reader
        // is seekable: a seekable reader can jump to `pos` directly regardless of whether the
        // atom iterator's bookkeeping still considers an atom "pending" (e.g. after a seek past
        // the last-known pending atom, or once the iterator has been exhausted).
        if !self.reader.is_seekable() {
