...
fn expand_backfill_items(
    ...
    match item.cache_kind.as_str() {
        "album" => {
            Ok(
                cover_backfill_items_for_album(store, library_server_id, &item.cache_entity_id)?
                    .into_iter()
        ...
        "artist" => {
            Ok(
                resolve_artist_cover_entry(store, library_server_id, &item.cache_entity_id)?
                    .into_iter()
                    .map(dto_to_backfill_item)
                    .collect(),
            )
...
pub use snapshot::{
    collect_missing_cover_targets, diff_missing_against_snapshot, snapshot_cover_disk,
    CoverDiskSnapshot,
};

pub fn count_distinct_cover_ids(
    store: &LibraryStore,
    library_server_id: &str,
) -> Result<i64, String> {
    store.with_read_conn(|conn| {
...
pub fn collect_cover_backfill_batch(
    ...
    let cover_ids = pending.iter().map(|i| i.cache_entity_id.clone()).collect();
...
pub fn collect_cover_progress_accurate(
    ...
    let total = count_distinct_cover_ids(store, library_server_id)?;
    let pending =
        count_pending_canonical_covers(store, library_server_id, cover_root, server_index_key)?;
