...
describe('offlineBrowseRouting', () => {
  ...
  it('isPathOfflineBrowsable covers library, detail, and local-only routes', () => {
    const ctx = [true, true, true] as const;
    ...
    expect(isPathOfflineBrowsable('/playlists', ...ctx)).toBe(false);
  });

  it('resolveOfflineDisconnectNavAction stays when nothing offline', () => {
    expect(resolveOfflineDisconnectNavAction('/playlists', false, false, false, false))
      .toEqual({ kind: 'stay' });
    expect(resolveOfflineDisconnectNavAction('/albums', false, false, false, false))
      .toEqual({ kind: 'stay' });
  });

  it('resolveOfflineDisconnectNavAction reloads allowed pages', () => {
    expect(resolveOfflineDisconnectNavAction('/artists', false, true, false, false))
      .toEqual({ kind: 'stay-reload' });
  });

  it('resolveOfflineDisconnectNavAction redirects disallowed pages to all albums', () => {
    expect(resolveOfflineDisconnectNavAction('/playlists', false, true, false, false))
      .toEqual({ kind: 'redirect', to: '/albums' });
    expect(resolveOfflineDisconnectNavAction('/', false, true, false, false))
      .toEqual({ kind: 'redirect', to: '/albums' });
  });
});
