export const help = {
  ...
  a2: 'Settings → Servers → Add Server. Enter the URL (e.g. http://192.168.1.100:4533), username, and password. Psysonic tests the connection before saving — nothing is stored if it fails. You can add as many servers as you like and browse several of them at the same time — see \'Can I use several servers at once?\' below.',
  ...
  q46: 'Can I use several servers at once?',
  a46: 'Yes. Settings → Servers holds every profile, and the library scope in the sidebar decides which of them you are browsing — pick music folders from several servers at the same time and Psysonic merges them into one catalogue in the priority order you set. Home, Albums, Artists, Composers, Genres, Favorites, Playlists, Folder Browser, Search, Most Played and Statistics all aggregate the selected servers, and music you hold on more than one server is de-duplicated instead of appearing once per server. A single queue can mix tracks from different servers; every track resolves its stream, cover, lyrics and ReplayGain against the server that owns it.',
  ...
  a5: 'Gapless pre-buffers the next track so there is zero silence between songs (best for live albums and DJ mixes). Crossfade fades the current track out while the next fades in over 1–10 s (best for shuffled mixes). AutoDJ is crossfade that follows the music: it trims the silence at the track edges and takes the overlap from content analysis, so the blend lands where the songs really end and start. Only one of the three can be active — turning one on turns the others off. Switch them from the queue toolbar, or configure them in Settings → Audio.',
  q6: 'What is Infinite Queue?',
  a6: 'When the queue runs out and Repeat is off, Psysonic silently appends similar / random tracks from your library so playback never stops. Auto-added tracks appear below a "— Added automatically —" divider. Toggle it with the infinity icon in the queue toolbar or in the mini player.',
  ...
  q47: 'Timeline queue view and Play from Here?',
  a47: 'Settings → Personalisation offers a Timeline layout for the queue: the current track stays at the top, what you already played scrolls above it, and what is coming stays below. Right-click a past entry and choose Play from Here to replay that moment — that track, every history entry after it and the existing Up Next list, in their original order and from the servers they came from.',
  ...
  q48: 'Can I cap the streaming bitrate?',
  a48: 'Per server address, under Settings → Servers when you edit a saved Navidrome profile. Choose no cap or a ceiling from 320 down to 64 kbps, with Auto, MP3, Opus or AAC as the target format, and Navidrome transcodes the live stream to match. Because the setting hangs off the address, a LAN endpoint and a public one can use different profiles and playback follows whichever you are connected to. Offline downloads, synced favourites and the hot cache always fetch the original file, so cached music keeps its full quality.',
  ...
  q49: 'Grid or table on the album pages?',
  a49: 'All Albums, New Releases and Lossless can switch between the card grid and a table, and each page remembers its own choice. The table shows cover, title, artist, song count, year, duration and the date an album was added, sorts by title or year from the column headers, and drops columns from the right when the window gets narrow.',
  q50: 'My playlists and the ones other people share?',
  a50: 'A Subsonic server hands over your own playlists and every public playlist of everyone else in one undivided list. The Playlists page can split that into your own, the ones you share, and the ones others share with you — the switch appears once something on the server is actually shared, and it works together with the folder view and the search box. The sidebar list shows each playlist cover and its song count, and both the sidebar and the page can be ordered by name, creation date or number of songs.',
  q51: 'What is the Composers page?',
  a51: 'Composers (sidebar) lists the composers found in your library tags and opens a detail page per composer with the works they are credited on. It follows the same library scope as the rest of the app, so with several servers selected it aggregates them into one list.',
  // ── Section 5: Lyrics ──────────────────────────────────────────────────
  s5: 'Lyrics',
  ...
  q52: 'Audio visualizer?',
  a52: 'Settings → Appearance → Visualizer turns it on for Now Playing, for the fullscreen player, or for both — each has its own switch. Pick a spectrum, oscilloscope, radial scope or stereo field, take the colours from the cover or from your theme, and adjust sensitivity, response, frame rate and peak markers. Internet radio is included while its equalizer graph is active.',
  q53: 'Where do I get more themes?',
  a53: 'Settings → Themes has the Theme Store: it lists community themes, shows a preview of each, and installs, updates or removes them in place. The catalogue and its previews are loaded from GitHub and no personal data is sent. Themes you install sit alongside the built-in ones in the same picker.',
  q54: 'Can I change the background images?',
  a54: 'Settings → Integrations → Backgrounds lets you decide which image the Mainstage hero, the artist page header and the fullscreen player use, and in which order the sources are tried — banner, fanart artist image or the image from your server. Drag to reorder, or switch a source off to skip it entirely.',
