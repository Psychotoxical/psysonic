export const help = {
  ...
  a39: 'Einstellungen → Integrationen → Music Network. Verbinde einen oder mehrere Scrobble-Dienste (Last.fm, Libre.fm, Rocksky, ListenBrainz, Maloja oder einen beliebigen GNU-FM-kompatiblen Server) — keine Navidrome-Konfiguration nötig. Jeder verbundene Dienst mit aktiviertem Scrobbeln erhält den Play; der Hauptschalter „Scrobbeln aktivieren" schaltet das ganze Fan-out an oder aus. Ein Scrobble wird bei dem in Music Network eingestellten Prozentsatz gesendet (Standard: 50 %); Now-Playing-Pings gehen zu Beginn raus, sofern der Dienst sie unterstützt. Ein Dienst ist dein Hauptdienst — geliebte Titel, ähnliche Künstler und Hörstatistiken kommen von ihm. Tipp: Wenn dein Navidrome-Server bereits zu einem Dienst scrobbelt (z. B. Maloja), verbinde denselben Dienst nicht zusätzlich hier, sonst entstehen doppelte Scrobbles.',
